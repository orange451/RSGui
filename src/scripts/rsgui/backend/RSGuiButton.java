...
public class RSGuiButton extends RSGuiNode implements RSGuiMouseListener {
	...
	public RSGuiButton(BufferedImage image) {
		super(0, 0, 1, 1);
		this.text = "";
		this.icon = image;
		GENERATE_BUTTON();
	...
	private void GENERATE_BUTTON() {
		int stringWid = RSGuiRes.FONT_REGULAR.getWidth(this.text) + (icon!=null?icon.getWidth():0);
		this.width = (stringWid + padding*2);
		this.height = 16 + padding*2;

		if ((this.buttonImage == null) || (this.buttonImage.getWidth() != this.width)
				|| (this.buttonImage.getHeight() != this.height)) {
			this.buttonImage = new BufferedImage(this.width, this.height, 2);
		}
	}
	...
	protected void paint(Graphics g) {
		...
		int xOff = 0;
		if ( icon != null ) {
			g2.drawImage(icon, width/2-icon.getWidth()/2, height/2-icon.getHeight()/2, null);
			xOff += icon.getWidth();
		}
		
		g2.setColor(Color.BLACK);
		g2.drawString(this.text, xOff + padding+1, this.height / 2 + padding+1);
		g2.setColor(this.hover ? this.selectColor : this.disabled ? Color.gray : this.normalColor);
		g2.drawString(this.text, xOff + padding, this.height / 2 + padding);
